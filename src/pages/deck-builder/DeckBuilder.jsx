...
export const DeckBuilder = (props) => {
    ...
    const cardsInDeck = Object.values(deckListCounter).reduce((a, b) => a + b, 0);
    ...
    const removeFromDeck = (card) => {
        if (deckListCounter[card._id]) {
            if (deckListCounter[card._id] > 0 && deckListCounter[card._id] <= 2) {
                const count = deckListCounter[card._id] - 1;
                setDeckListCounter({ ...deckListCounter, [card._id]: count });
            }
    ...
    const handleSaveDeck = () => {
        doCall('http://localhost:5000/decks', { data: { name: 'test', cards: ['60040163fa69ed68329eedbf', '6004b67016b41e01eaa8138e'] }, method: 'POST' }).then((res) => {
            debugger;
        });
    };
    ...
    return (
        <div>
            <div className='flex'>
                <div className='ml-3 mr-5 p-3 bg-medium-blue rounded flex flex-col justify-between' style={{ minWidth: '300px', maxHeight: 'calc(100vh - 135px)' }}>
                    <div>
                        ...
                        <div className='bg-medium-blue '>
                            {deckList.map((card, i) => {
                                return (
                                    <div key={`${card._id}-${i}`} className='flex items-center'>
                                        <div onClick={() => removeFromDeck(card)} className='w-full bg-dark-blue rounded flex items-center mt-2 cursor-pointer no-select'>
                                            <img
                                                className='object-cover rounded-l'
                                                style={{ width: '30px', height: '30px' }}
                                                src={'/' + card.imagePath.split('.')[0] + '-tooltip' + '.' + card.imagePath.split('.')[1]}
                                            />
                                            <div className='w-full flex justify-between px-2'>
                                                <div className='flex items-center'>
                                                    <div className=''>{deckListCounter[card._id]}x</div>
                                                    <div className='ml-2 text-white'>{card.name}</div>
                                                </div>
                                            </div>
                                            <div className='flex items-center px-2 ' style={{ height: '30px' }}>
                                                <div>{card.mana}</div>
                                                <div className='ml-1'>
                                                    <Mana />
                                                </div>
                ...
                <div className='flex flex-wrap overflow-y-scroll mr-3' style={{ minWidth: '250px', maxHeight: 'calc(100vh - 135px)' }}>
                    {cards.map((card) => {
                        return (
                            <div onClick={() => handleAddToDeck(card)} key={card._id} className={`mb-5 mr-5 ${deckListCounter[card._id] === 2 || cardsInDeck === 30 ? 'opacity-30' : ''}`}>
